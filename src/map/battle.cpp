...
static int battle_calc_attack_skill_ratio(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	case WM_GREAT_ECHO:
		...
		break;
	case GN_CART_TORNADO:
	{ // ATK [( Skill Level x 100 ) + ( Cart Weight / ( 150 - Caster Base STR ))] + ( Cart Remodeling Skill Level x 50 )] %
		skillratio += -100 + 100 * skill_lv;
		if (sd && sd->cart_weight)
...
static void battle_attack_sc_bonus(struct Damage *wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sc)
	{
		...
		if (sc->data[SC_SPIRIT])
		{
			...
			{
				ATK_ADDRATE(wd->damage, wd->damage2, map_flag_gvg2(src->m) ? 25 : 50); //+25% dmg on woe/+100% dmg on nonwoe
				RE_ALLATK_ADDRATE(wd, map_flag_gvg2(src->m) ? 25 : 50);				   //+25% dmg on woe/+100% dmg on nonwoe
			}
...
static const struct _battle_data
{
	...
	{
		"ignore_items_gender",
		&battle_config.ignore_items_gender,
		1,
		0,
		1,
	},
	...
	{
		"exp_cost_inspiration",
		&battle_config.exp_cost_inspiration,
		1,
		0,
		100,
	},
	...
	{
		"event_refine_chance",
		&battle_config.event_refine_chance,
		0,
		0,
		1,
	},
